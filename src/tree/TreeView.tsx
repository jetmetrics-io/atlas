...
export function TreeView({ slug, onBack, initialDrill }:
  ...
  const tx0 = drill && B
    // Колонка начинается не в нуле раскладки, поэтому её смещение (B.cols[0])
    // входит в расчёт: без него подпись всё равно уезжала под чип.
    // max, а не min: правило «не заезжать под чипы» важнее, чем прижать схему
    // к правому краю. Схема шире экрана в любом случае, её листают.
    ? Math.max(DRILL_LEFT - (A.w + TREE_GAP + B.cols[0]) * z, box.w - PAD - cw * z)
    : (box.w - A.w * z) / 2
