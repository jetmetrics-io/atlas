...
def stamp_date():
    """Дата релиза в atlas_base.json — только для сборки `npm run gen:legacy`.

    ⚠️ Каталог её больше НЕ показывает. С переездом на Базу 26.08.2026 данные
    приложения собирает `content/db/build_app.py`, и дату «Обновлено» ставит он —
    днём сборки, а сборка всегда предшествует выкладке. Пока это не заметили,
    каталог три дня показывал 26 августа: деплой честно штамповал файл, который
    приложение уже не читает.

    Функция оставлена, чтобы легаси-путь не разъехался, но на бой она не влияет.
    """
