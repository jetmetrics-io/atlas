...
// Правки к Базе вносятся прямо в atlas_base.json. Слой fixes.json убран 26.08.2026:
// 47 правок влиты в Базу, файл лежит в content/archive/. Слой был нужен, пока правки
// нельзя было вносить в саму Базу: `build.py init` пересобирает рабочую таблицу и берёт
// колонки «Метрика, Роль, Ед., Формула, Описание» из Базы заново, поэтому правка,
// сделанная в таблице, при следующей пересборке возвращалась. Теперь правка идёт в Базу
// и переживает init по определению. История правок: content/maps/issues.md.
// (До 21.08.2026 слой применял только build.py, и 57 полей в бою оставались неисправленными —
//  теперь такого расхождения быть не может: у Базы и таблицы один источник.)
